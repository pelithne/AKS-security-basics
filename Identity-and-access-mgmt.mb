...
### Secure cluster acces with Microsoft Entra ID (Azure Active Directory)
...
````bash
az aks update -g <Resource group> -n <AKS Cluster Name> --enable-azure-rbac --enable-aad --disable-local-accounts
````



Then grant the **Admin** group users the permissions to connect to and manage all aspects of the AKS cluster.
...
````bash
az ad user create --display-name <Admin role display name>  --user-principal-name <user principal name> --password <admin password>
````
...
````bash
admin_user_object_id=$(az ad user show --id <user principal name> --query 'id' --output tsv)
````

Assign the user to the admin security group.

````bash
az ad group member add --group <Admin group name> --member-id $admin_user_object_id
````

### Validate the access to the cluster.

````
az aks get-creadentials -g <Resource Group> -n <AKS Cluster Name>
````
